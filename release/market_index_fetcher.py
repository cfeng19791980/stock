        ...
        if exists > 0:
            ...
            cursor.execute('''
                UPDATE index_daily SET 
                    open=?, close=?, high=?, low=?, volume=?, pct_chg=?, amount=?
                WHERE code=? AND date=?
            ''', (
                realtime_data['open'], realtime_data['close'],
                realtime_data['high'], realtime_data['low'],
                realtime_data.get('volume', 0), realtime_data['pct_chg'],
            ...
            # 插入今日数据
            cursor.execute('''
                INSERT INTO index_daily (code, date, open, close, high, low, volume, pct_chg)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
...
if __name__ == '__main__':
    ...
    print(f"数据状态: {freshness['latest_date']} (延迟{freshness['delay_days']}天)")
    ...
    print("最近5天沪深300数据:")
    for r in recent:
        print(f"  {r[0]}: 收盘={r[1]:.2f}, 涨跌={r[2]:.2f}%")
    
    # 检查今日数据
    today_check = datetime.now().strftime('%Y-%m-%d')
    cursor.execute('SELECT close FROM index_daily WHERE code="sh.000300" AND date=?', (today_check,))
    today_data = cursor.fetchone()
    
    if today_data:
        print(f"\n✓ 今日数据完整: {today_check} 收盘={today_data[0]:.2f}")
    else:
        print(f"\n✗ 今日数据缺失: {today_check}")
