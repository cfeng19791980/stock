# -*- coding: utf-8 -*-
"""
波段股票分析系统 - JSON输出版（完整版v3.1）
架构: Python分析引擎 → JSON文件 → Electron前端读取
版本: v3.1-improved（完整43特征 + 改进买入卖出逻辑）
改进:
  - 买入信号: 涨幅>=3%概率>60%
...
print("=" * 70)
print("波段股票分析系统 v3.1 (完整43特征 + 改进逻辑)")
print("最高原则: 准确性第一")
print("=" * 70)
...
for code in stock_pool:
    ...
    if feat is None:
        ...
        if model:
            try:
                ...
                if pred[1] > 0.6:
                    action = "买入"
                    score = int(pred[1] * 100)  # 60-100，反映买入置信度
                
                # 卖出信号：涨幅>=3%概率<15%（跌幅概率>85%）
                elif pred[1] < 0.15:
                    action = "卖出"
                    # 卖出评分反映跌幅置信度
                    sell_confidence = 1 - pred[1]  # 跌幅概率
                    score = int(sell_confidence * 100)  # 85-100
    ...
    # 买入卖出筛选（改进）
    # 买入：评分>=60且action=买入（已自然满足）
    if action == "买入" and score >= 60:
        buy_list.append(result)
    # 卖出：评分>=85且action=卖出（已自然满足）
    elif action == "卖出" and score >= 85:
        sell_list.append(result)

print(f"✓ 分析完成: {len(results)}只股票")
print(f"✓ 买入信号: {len(buy_list)}只（涨幅>=3%概率>60%）")
print(f"✓ 卖出信号: {len(sell_list)}只（跌幅>=3%概率>85%）")

# 输出JSON
# 排序后输出（修复Bug）
buy_list_sorted = sorted(buy_list, key=lambda s: -s['score'])
sell_list_sorted = sorted(sell_list, key=lambda s: -s['score'])
...
output = {
    ...
    'buy_signal_threshold': '涨幅>=3%概率>60%',
    'sell_signal_threshold': '涨幅>=3%概率<15%（跌幅概率>85%）',
    'stocks': results,
    'buy': buy_list_sorted[:5],  # TOP 5买入推荐（按评分降序）
    'sell': sell_list_sorted
}
