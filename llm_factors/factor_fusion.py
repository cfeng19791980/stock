...
def get_tech_score(row):
    ...
    # 3. 量能分
    vr = row['volume_ratio'] if pd.notna(row.get('volume_ratio', 0)) else 1.0
    if 0.8 <= vr <= 1.4:
        vol = 70
    elif vr > 1.4:
        vol = 40
...
def get_fusion_score(code, latest_row, date=None, session='afternoon', fund_signals=None):
    ...
    if has_anomaly and conf >= 0.6:
        ...
        # 无实时异动，传统LLM因子
        final = tech_score * 0.55 + news_score_adj * 0.15 + fund_sig['fin'] * 0.20 + fund_sig['fund'] * 0.10
    elif conf >= 0.3:
        final = tech_score * 0.65 + news_score_adj * 0.10 + fund_sig['fin'] * 0.15 + fund_sig['fund'] * 0.10
    else:
        final = tech_score * 0.80 + news_score_adj * 0.10 + fund_sig['fin'] * 0.10
