...
def generate_trade_advice(score, adjusted_score=None, market_status=None, factor=None, buy_threshold=60, sell_threshold=15, suggest_position=0.15):
    """生成买卖建议（非持仓股）- 自适应版"""
    adj_info = ""
    ...
    # 使用动态阈值
    pos_pct = int(suggest_position * 100)
    
    if score >= 80:
        return f"强烈推荐买入！评分{score}分{adj_info}。建议优先配置{pos_pct}%仓位。"
    elif score >= buy_threshold + 10:
        return f"推荐买入。评分{score}分{adj_info}。建议分批建仓。"
    elif score >= buy_threshold:
        return f"可考虑买入。评分{score}分{adj_info}，达到买入阈值({buy_threshold}分)。"
    elif score >= sell_threshold + 10:
        return f"暂不买入。评分{score}分{adj_info}，未达买入阈值。"
    elif score >= sell_threshold:
        return f"观望。评分{score}分{adj_info}，中性区间。"
    else:
        return f"建议卖出！评分仅{score}分{adj_info}。如有持仓建议止损。"

def generate_holding_advice(profit, score, days, holding, stop_profit=25, stop_loss=-10, suggest_position=0.15):
    """生成持仓建议（持仓股）- 自适应版"""
    ...
    if profit >= stop_profit:
        return f"✅ 已达止盈线({stop_profit}%+)！盈利{profit:.1f}%。建议分批卖出锁定收益。"
    elif profit >= stop_profit - 5:
        return f"接近止盈线。盈利{profit:.1f}%。建议卖出30-50%仓位。"
    elif profit >= 5:
        return f"小幅盈利({profit:.1f}%)。持仓{days}天。继续持有。"
    elif profit >= stop_loss / 2:
        return f"轻微亏损({profit:.1f}%)。继续观察，止损线{stop_loss}%。"
    elif profit >= stop_loss:
        return f"⚠️ 接近止损线({stop_loss}%+)！亏损{profit:.1f}%。密切关注。"
    else:
        return f"🔴 触发止损线！亏损{profit:.1f}%（超{stop_loss}%）。建议果断止损。"
...
# ========== 获取大盘状态 ==========
factor, market_pct, market_status, buy_threshold, sell_threshold, stop_profit, stop_loss, suggest_position, recent_5d, indices_data, recent_zz500 = get_market_adjustment(conn)
print(f"\n大盘状态: {market_status}, 5日涨跌: {market_pct:.2f}%")
print(f"自适应策略: 买入阈值{buy_threshold}分, 卖出阈值{sell_threshold}分")
print(f"止盈止损: 止盈{stop_profit}%+, 止损{stop_loss}%")
print(f"建议仓位: {suggest_position*100:.0f}%")
if recent_5d:
    print("最近5日走势:")
    for r in recent_5d:
...
MODEL_CACHE_DIR = r'E:\csi10\model_cache'
MODEL_CACHE_FILE = os.path.join(MODEL_CACHE_DIR, 'models.pkl')
MODEL_META_FILE = os.path.join(MODEL_CACHE_DIR, 'meta.json')

def load_cached_models():
    """加载缓存模型"""
    try:
        if os.path.exists(MODEL_CACHE_FILE) and os.path.exists(MODEL_META_FILE):
            ...
            if meta.get('data_date') == current_date:
...
def save_models_cache(models, data_date):
    """保存模型缓存"""
    try:
        os.makedirs(MODEL_CACHE_DIR, exist_ok=True)
        with open(MODEL_CACHE_FILE, 'wb') as f:
            pickle.dump(models, f)
        meta = {'data_date': data_date, 'model_count': len(models), 'created': datetime.now().isoformat()}
...
cursor = conn.cursor()
cursor.execute('SELECT MAX(date) FROM daily_price')
current_data_date = cursor.fetchone()[0]

# 尝试加载缓存
cached_models, cached_meta = load_cached_models()
if cached_models and cached_meta:
    print(f"✓ 使用缓存模型: {len(cached_models)}个 (数据日期: {cached_meta.get('data_date')})")
    models = cached_models
else:
    print("缓存失效或不存在，重新训练模型...")
    models = {}
    ...
    # 保存缓存
    save_models_cache(models, current_data_date)
    print(f"✓ 已训练并缓存: {len(models)}个模型")

print(f"✓ 模型就绪: {len(models)}个")
...
for code in stock_pool:
    ...
    try:
        ...
        if holding:
            ...
            advice = generate_holding_advice(profit, score, hold_days, holding, stop_profit, stop_loss, suggest_position)
        else:
            # 非持仓股：生成买卖建议（包含大盘调整信息）
            adjusted = adjust_score(score, factor)
            advice = generate_trade_advice(score, adjusted, market_status, factor, buy_threshold, sell_threshold, suggest_position)
        ...
        stocks_analysis.append({
            ...
            'advice': advice,
            'buy_signal': bool(adjust_score(score, factor) >= buy_threshold and not holding),  # 使用自适应买入阈值
            'sell_signal': bool(score < sell_threshold or (holding and profit <= stop_loss)),  # 使用自适应卖出阈值
...
print(f"\n大盘调整: {market_status} | 因子{factor} | 买入阈值{buy_threshold}分 卖出阈值{sell_threshold}分")
